...
local function toggle(
    ...
    row.SetValue = function(value, fireCallback)
        enabled = value == true
        render(false)
...
local function slider(
    ...
    holder.SetValue = function(newValue, fireCallback)
        value = math.clamp(
        ...
        local snapped = math.floor(((value - minimum) / step) + 0.5) * step + minimum
...
local function createGameTestingFeatures(window)
    ...
    local function updateScreenEsp(target, data)
        if not state.ESP or not data or not data.Character or not data.Character.Parent then
            if data then
                data.ScreenBox.Visible = false
                data.Tracer.Visible = false
            end
        ...
        for _, cornerPosition in ipairs(corners) do
            local screenPoint, visible = camera:WorldToViewportPoint(cornerPosition)
        ...
        if visibleCount == 0 or minX == math.huge then
            data.ScreenBox.Visible = false
            data.Tracer.Visible = false
            return
        end
        ...
        data.ScreenBox.Position = UDim2.fromOffset(minX, minY)
        data.ScreenBox.Size = UDim2.fromOffset(width, height)
        data.ScreenBox.Visible = state.ESPBox

        local viewport = camera.ViewportSize
        local startPoint = Vector2.new(viewport.X * 0.5, viewport.Y)
        local endPoint = Vector2.new((minX + maxX) * 0.5, maxY)
        local delta = endPoint - startPoint
        local length = delta.Magnitude

        if state.ESPTracer and length > 1 then
            data.Tracer.Position = UDim2.fromOffset(startPoint.X, startPoint.Y)
    ...
    local function applyConfig(data)
        ...
        local function applyControl(name, value)
            ...
            if control and type(control.SetValue) == "function" and value ~= nil then
