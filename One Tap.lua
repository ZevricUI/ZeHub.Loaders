...
local function toggle(
    ...
    -- Roblox Instances cannot have arbitrary custom members such as
    -- `SetValue`. Return a small control wrapper instead of attaching
    -- methods directly to the Frame. This keeps the public control API
    -- working and makes config loading reliable.
    local control = {
        Root = row
    }
...
local function slider(
    ...
    -- Return a control wrapper; Roblox Frames do not support arbitrary
    -- members, so assigning `holder.SetValue` would leave SetValue missing.
    local control = {
        Root = holder
    }

    function control:SetValue(newValue, fireCallback)
        value = math.clamp(
        ...
        local snapped =
            math.floor(((value - minimum) / step) + 0.5)
            * step
            + minimum
...
local function addBuiltInTabs(window, gameState)
    ...
    local profileCard = createUtilityCard(profileSection.Root, 88)
    ...
    local avatarStroke = stroke(avatar, Theme.Border, 0.25)

    local displayLabel = createUtilityLabel(
        profileCard,
        "",
        UDim2.fromOffset(88, 13),
        UDim2.new(1, -100, 0, 22),
        14,
        Theme.Text,
        true
    )
    ...
    local statusLabel = createUtilityLabel(
        profileCard,
        "● Active",
        UDim2.fromOffset(88, 57),
        UDim2.new(1, -100, 0, 18),
        10,
        Theme.Muted,
        true
    ...
    local detailsSection = profileTab:CreateSection({
        Name = "Account"
    })

    detailsSection:CreateFeatureCard({
        Title = "One Tap",
        Items = {
            "User ID: " .. tostring(player.UserId),
            "Display: " .. tostring(player.DisplayName or player.Name),
            "Username: @" .. tostring(player.Name),
            "Session profile is stored locally."
        }
    })
    ...
    profileSection:CreateButton({
        ...
        Callback = function()
            displayLabel.Text = state.HideUsername and "ZeHub Player" or tostring(player.DisplayName or player.Name)
            usernameLabel.Text = state.HideUsername and "Username hidden" or ("@" .. tostring(player.Name))
            avatar.ImageTransparency = state.HideAvatar and 1 or 0
            statusLabel.Text = "● Active"
        end
    })

    local changelogSection = profileTab:CreateSection({
        Name = "Changelog"
    })
    ...
    changelogSection:CreateFeatureCard({
        ...
        Items = {
            ...
            "Removed the redundant Info tab.",
            "Added working feature Save / Load configs.",
            "Configs restore toggles and slider values."
        }
    })
    ...
    configSection:CreateParagraph({
        Text = "Save your current in-game feature settings to one of three local slots. Loading a slot applies the settings immediately.",
        Height = 58
    })

    local configStatus = configSection:CreateParagraph({
        Text = "Status: Ready • Slots are stored locally as JSON.",
        Height = 42
    })

    local function slotButtons(slot)
        configSection:CreateButton({
            Name = "Save Config " .. tostring(slot),
            Callback = function()
                if gameState and gameState.SaveConfig then
                    local ok, message = gameState.SaveConfig(slot)
                    local messageText = ok and ("Status: Saved Config " .. tostring(slot)) or "Status: Save failed"
                    statusLabel.Text = ok and ("● Saved Config " .. tostring(slot)) or "● Save failed"
                    configStatus.Text = messageText
                    if ok then
                        task.delay(2, function()
                            if statusLabel.Parent then statusLabel.Text = "● Active" end
                        end)
                    end
                end
            end
        ...
        configSection:CreateButton({
            Name = "Load Config " .. tostring(slot),
            Callback = function()
                if gameState and gameState.LoadConfig then
                    local ok = gameState.LoadConfig(slot)
                    local messageText = ok and ("Status: Loaded Config " .. tostring(slot)) or "Status: Load failed"
                    statusLabel.Text = ok and ("● Loaded Config " .. tostring(slot)) or "● Load failed"
                    configStatus.Text = messageText
                    if ok then
                        task.delay(2, function()
                            if statusLabel.Parent then statusLabel.Text = "● Active" end
    ...
    local appearanceSection = configTab:CreateSection({
        Name = "Appearance"
    })
    ...
    return {
        Profile = profileTab,
        Configs = configTab,
        HideUsername = hideUsernameToggle,
        HideAvatar = hideAvatarToggle
    }
...
local function createGameTestingFeatures(window)
    ...
    -- The ESP overlay must be declared before ensureEsp/destroyEsp so the
    -- screen-space box and tracer are parented to the actual ScreenGui.
    local overlayGui
    ...
    local function ensureEsp(target)
        ...
        if not data then
            ...
            -- Kept for compatibility with the original ESP object layout.
            -- The visible box is rendered in screen-space below so it remains
            -- reliable even when the character model changes.
            ...
            screenBox.Name = "ZeHubESP2DBox"
            screenBox.BackgroundTransparency = 1
            screenBox.BorderSizePixel = 0
            screenBox.Visible = false
            screenBox.ZIndex = 20
            screenBox.Parent = overlayGui

            local boxStroke = Instance.new("UIStroke")
            boxStroke.Color = Color3.new(1, 1, 1)
            boxStroke.Thickness = 1
            boxStroke.Transparency = 0
            boxStroke.Parent = screenBox

            local tracer = Instance.new("Frame")
            tracer.Name = "ZeHubESPTracer"
    ...
    overlayGui.DisplayOrder = 32767
    overlayGui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    overlayGui.Enabled = true

    -- Use the same GUI parent as ZeHub itself so the ESP overlay is not
    -- trapped behind a different CoreGui/PlayerGui layer.
    overlayGui.Parent = guiParent

    local function updateScreenEsp(target, data)
        if not state.ESP
            or not data
        ...
        -- Prefer the model's real bounds. This works with R6, R15 and
        -- custom character rigs.
        ...
        if ok and boxCFrame and boxSize then
            ...
            local corners = {
                boxCFrame:PointToWorldSpace(Vector3.new(-half.X, -half.Y, -half.Z)),
                boxCFrame:PointToWorldSpace(Vector3.new(-half.X, -half.Y,  half.Z)),
                boxCFrame:PointToWorldSpace(Vector3.new(-half.X,  half.Y, -half.Z)),
                boxCFrame:PointToWorldSpace(Vector3.new(-half.X,  half.Y,  half.Z)),
                boxCFrame:PointToWorldSpace(Vector3.new( half.X, -half.Y, -half.Z)),
            ...
            for _, worldPosition in ipairs(corners) do
                ...
                -- Z is more reliable than the boolean for determining
                -- whether a point is in front of the camera.
                if point.Z > 0 then
        ...
        -- Fallback for partially off-screen/custom rigs: build a box from
        -- the root and head positions so ESP does not silently disappear.
            ...
            if root then
                ...
                if rootPoint.Z > 0 then
                    ...
                    if topPoint.Z > 0 then
                        ...
                        minX = rootPoint.X - width * 0.5
                        maxX = rootPoint.X + width * 0.5
                        minY = math.min(
                            rootPoint.Y,
                            topPoint.Y
                        )
        ...
        -- Clamp the rectangle to the viewport while preserving a visible
        -- two-pixel minimum.
        local viewport = camera.ViewportSize

        minX = math.clamp(minX, -viewport.X, viewport.X * 2)
        maxX = math.clamp(maxX, -viewport.X, viewport.X * 2)
        minY = math.clamp(minY, -viewport.Y, viewport.Y * 2)
        maxY = math.clamp(maxY, -viewport.Y, viewport.Y * 2)

        local width = math.max(2, maxX - minX)
        local height = math.max(2, maxY - minY)

        data.ScreenBox.Position = UDim2.fromOffset(
            minX,
            minY
        )

        data.ScreenBox.Size = UDim2.fromOffset(
            width,
            height
        )

        data.ScreenBox.Visible = state.ESPBox
        ...
        local tracerLength = delta.Magnitude

        if state.ESPTracer and tracerLength > 1 then
    ...
    local configFolder = "ZeHub"
    local configPrefix = configFolder .. "/OneTap_Config_"

    local function ensureConfigFolder()
        if type(isfolder) == "function" then
            local ok, exists = pcall(isfolder, configFolder)
            if ok and not exists and type(makefolder) == "function" then
    ...
    local function configFile(slot)
        return configPrefix .. tostring(slot) .. ".json"
    end
    ...
    local function getConfigData()
        return {
            Aimbot = state.Aimbot,
            AimFOV = state.AimFOV,
            AimSmooth = state.AimSmooth,
            Triggerbot = state.Triggerbot,
            TriggerDelay = state.TriggerDelay,
            FOV = state.FOV,
            FOVSize = state.FOVSize,
            ESP = state.ESP,
    ...
    local function saveConfig(slot)
        ...
        local wrote = pcall(writefile, configFile(slot), encoded)
    ...
    local function applyConfig(data)
        ...
        local function applyControl(name, value)
            local control = controls[name]

            if control
                and type(control.SetValue) == "function"
                and value ~= nil then
                ...
                if not ok then
                    -- Keep a malformed individual setting from preventing
                    -- the rest of a config from loading.
                    return false
                end
        ...
        applyControl("Aimbot", data.Aimbot)
        applyControl("AimFOV", data.AimFOV)
        applyControl("AimSmooth", data.AimSmooth)
        applyControl("Triggerbot", data.Triggerbot)
        applyControl("TriggerDelay", data.TriggerDelay)
        applyControl("FOV", data.FOV)
        applyControl("FOVSize", data.FOVSize)
        applyControl("ESP", data.ESP)
        applyControl("ESPName", data.ESPName)
        applyControl("ESPDistance", data.ESPDistance)
        applyControl("ESPHealth", data.ESPHealth)
        applyControl("ESPBox", data.ESPBox)
        applyControl("ESPTracer", data.ESPTracer)
        applyControl("Fly", data.Fly)
        applyControl("FlySpeed", data.FlySpeed)
        applyControl("Noclip", data.Noclip)
        applyControl("JumpPower", data.JumpPower)
    ...
    local function loadConfig(slot)
        ...
        return applyConfig(data), "Loaded"
    end

    state.SaveConfig = saveConfig
    state.LoadConfig = loadConfig

    local oneTapTab = window:CreateTab({
        Name = "Aimbot",
        Icon = "Stats"
    })

    local aimbotSection = oneTapTab:CreateSection({
        Name = "Aimbot"
    })

    controls.Aimbot = aimbotSection:CreateToggle({
        Name = "Aimbot",
        Default = false,
        Callback = function(value)
    ...
    controls.AimFOV = aimbotSection:CreateSlider({
        Name = "Aim FOV",
        Min = 20,
        Max = 360,
    ...
    controls.AimSmooth = aimbotSection:CreateSlider({
        Name = "Aim Smooth",
        Min = 0.02,
        Max = 1,
    ...
    controls.Triggerbot = aimbotSection:CreateToggle({
        Name = "Triggerbot",
        Default = false,
        Callback = function(value)
    ...
    controls.TriggerDelay = aimbotSection:CreateSlider({
        Name = "Trigger Delay",
        Min = 0,
        Max = 2,
        Default = 0,
    ...
    controls.FOV = aimbotSection:CreateToggle({
        Name = "FOV Circle",
        Default = false,
        Callback = function(value)
    ...
    controls.FOVSize = aimbotSection:CreateSlider({
        Name = "FOV Size",
        Min = 20,
        Max = 360,
    ...
    controls.ESP = visualsSection:CreateToggle({
        Name = "ESP",
        Default = false,
        Callback = function(value)
    ...
    controls.ESPName = visualsSection:CreateToggle({
        Name = "ESP Name",
        Default = true,
        Callback = function(value)
    ...
    controls.ESPDistance = visualsSection:CreateToggle({
        Name = "ESP Distance",
        Default = true,
        Callback = function(value)
    ...
    controls.ESPHealth = visualsSection:CreateToggle({
        Name = "ESP Health",
        Default = true,
        Callback = function(value)
    ...
    controls.ESPBox = visualsSection:CreateToggle({
        Name = "ESP Box",
        Default = false,
        Callback = function(value)
    ...
    controls.ESPTracer = visualsSection:CreateToggle({
        Name = "ESP Tracer",
        Default = false,
        Callback = function(value)
            state.ESPTracer = value == true
            refreshEsp()
        end
    })
    ...
    controls.Fly = movementSection:CreateToggle({
        Name = "Fly",
        Default = false,
        Callback = function(value)
    ...
    controls.FlySpeed = movementSection:CreateSlider({
        Name = "Fly Speed",
        Min = 10,
        Max = 200,
    ...
    controls.Noclip = movementSection:CreateToggle({
        Name = "No Clip",
        Default = false,
        Callback = function(value)
    ...
    controls.JumpPower = movementSection:CreateSlider({
        Name = "Jump Power",
        Min = 0,
        Max = 200,
    ...
    connect(RunService.RenderStepped, function()
        ...
        if state.ESP then
            -- Creation/removal is handled by player/character events and by
            -- the toggle callback. RenderStepped only updates positions.
            for _, target in ipairs(Players:GetPlayers()) do
                if target ~= player then
                    local data = state.EspObjects[target]
                    if not data then
                        data = ensureEsp(target)
                    end
                    if data then
                        updateScreenEsp(target, data)
                    end
                end
            end

            for target, data in pairs(state.EspObjects) do
                if not target.Parent then
                    destroyEsp(target)
                elseif data and data.Character ~= target.Character then
                    destroyEsp(target)
                end
            end
        end
...
function Library:CreateWindow(config)
    ...
    -- Main in-game feature tabs are created first so they stay at the top of
    -- the sidebar. Utility pages are added underneath them.
    ...
    if config.BuiltInTabs ~= false then
        addBuiltInTabs(window, window._gameTestingState)
        refreshSideTabs()
    end
