...
local function safeFileAvailable()
...
local function ensureConfigFolder()
    if not safeFileAvailable() then return false end
    pcall(function() if not isfolder(CONFIG_FOLDER) then makefolder(CONFIG_FOLDER) end end)
    pcall(function() if not isfolder(CONFIG_PATH) then makefolder(CONFIG_PATH) end end)
    return isfolder(CONFIG_PATH)
end

local function configFile(name)
    return CONFIG_PATH .. "/" .. tostring(name):gsub("[^%w%-%_ ]", "") .. ".json"
end

local function registerControl(name, kind, getter, setter, extra)
...
local function snapshotControls()
    local result = {}
    for name, control in pairs(controlRegistry) do
        if control.Get then
            local ok, value = pcall(control.Get)
            if ok then result[name] = value end
        end
    end
    result.__profile = {
        HideUsername = profileSettings.HideUsername,
        HideAvatar = profileSettings.HideAvatar
    }
    result.__favorites = {}
    for name, enabled in pairs(favoriteRegistry) do
        if enabled then result.__favorites[name] = true end
    end
    return result
end

local function applyControlSnapshot(data, fireCallbacks)
    if type(data) ~= "table" then return false end
    for name, value in pairs(data) do
        local control = controlRegistry[name]
        if control and control.Set then
            pcall(control.Set, value, fireCallbacks == true)
        end
    end
    if type(data.__profile) == "table" then
        profileSettings.HideUsername = data.__profile.HideUsername == true
        profileSettings.HideAvatar = data.__profile.HideAvatar == true
    end
    if type(data.__favorites) == "table" then
        favoriteRegistry = {}
        for name, enabled in pairs(data.__favorites) do
            if enabled then favoriteRegistry[name] = true end
        end
    end
    return true
end
...
local function saveConfig(name)
    if not ensureConfigFolder() then return false, "File system is unavailable" end
    local clean = tostring(name or ""):gsub("^%s+", ""):gsub("%s+$", "")
    if clean == "" then return false, "Enter a config name" end
    local ok, encoded = pcall(HttpService.JSONEncode, HttpService, snapshotControls())
    if not ok then return false, "Could not encode config" end
    local path = configFile(clean)
    local wrote = pcall(writefile, path, encoded)
    return wrote, wrote and "Saved" or "Could not save config"
end

local function loadConfig(name, fireCallbacks)
    if not ensureConfigFolder() then return false, "File system is unavailable" end
    ...
    applyControlSnapshot(data, fireCallbacks)
    if type(refreshSidebarProfile) == "function" then refreshSidebarProfile() end
    return true, "Loaded"
end

local function listConfigs()
    if not ensureConfigFolder() or type(listfiles) ~= "function" then return {} end
    local result = {}
    for _, path in ipairs(listfiles(CONFIG_PATH)) do
...
local function deleteConfig(name)
    ...
    return ok, ok and "Deleted" or "Could not delete config"
end
...
-- Compact profile card anchored to the bottom of the sidebar.
local sidebarProfile = create("Frame", {
    Name = "ProfileCard",
...
corner(sidebarProfile, 5)
local sidebarProfileStroke = stroke(sidebarProfile, Theme.Border, 0.38)
local sidebarAvatar = create("ImageLabel", {
    Position = UDim2.fromOffset(5, 5), Size = UDim2.fromOffset(36,36),
    BackgroundColor3 = Theme.Input, BorderSizePixel = 0,
    Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(player.UserId) .. "&w=150&h=150", Parent = sidebarProfile
})
corner(sidebarAvatar, 18)
local sidebarProfileName = create("TextLabel", {
    Position = UDim2.fromOffset(48, 6), Size = UDim2.new(1,-54,0,17), BackgroundTransparency=1,
    TextColor3=Theme.Text, TextSize=10, Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left,
    TextTruncate=Enum.TextTruncate.AtEnd, Parent=sidebarProfile
})
local sidebarProfileSub = create("TextLabel", {
    Position = UDim2.fromOffset(48, 23), Size = UDim2.new(1,-54,0,14), BackgroundTransparency=1,
    TextColor3=Theme.Muted, TextSize=8, Font=Enum.Font.GothamSemibold, TextXAlignment=Enum.TextXAlignment.Left,
    TextTruncate=Enum.TextTruncate.AtEnd, Parent=sidebarProfile
})
refreshSidebarProfile = function()
    local hidden = profileSettings.HideUsername
    sidebarProfileName.Text = hidden and "ZeHub Player" or player.DisplayName
    sidebarProfileSub.Text = hidden and "Privacy enabled" or ("@" .. player.Name)
    sidebarAvatar.Visible = not profileSettings.HideAvatar
end
refreshSidebarProfile()
registerThemeRefresh(function()
    if sidebarProfile.Parent then
        sidebarProfile.BackgroundColor3=Theme.Row
        sidebarProfileStroke.Color=Theme.Border
        sidebarProfileName.TextColor3=Theme.Text
        sidebarProfileSub.TextColor3=Theme.Muted
        sidebarAvatar.BackgroundColor3=Theme.Input
    end
end)
...
local function toggle(
    ...
    registerControl(name, "Toggle", function()
        return enabled
    end, function(value, fire)
        enabled = value == true
        render(false)
        if fire and callback then task.spawn(callback, enabled) end
    end)
...
local function dropdown(
    ...
    registerControl(name, "Dropdown", function()
        return selected
    end, function(value, fire)
        for _, option in ipairs(options) do
            if option == value then
                ...
                refreshOptions()
                if fire and callback then task.spawn(callback, option) end
                return
            end
        end
    end, {Options = options})
...
local function multiDropdown(
    ...
    registerControl(name, "MultiDropdown", function()
        return snapshot()
    end, function(value, fire)
        selected = {}
        ...
        refresh()
        if fire and callback then task.spawn(callback, snapshot(), isAllSelected()) end
    end, {Options = options})
...
local function slider(
    ...
    registerControl(name, "Slider", function()
        return value
    end, function(newValue, fire)
        value = math.clamp(tonumber(newValue) or minimum, minimum, maximum)
        value = math.clamp(math.floor((value - minimum) / step + 0.5) * step + minimum, minimum, maximum)
        render()
        if fire and callback then task.spawn(callback, value) end
    end, {Minimum = minimum, Maximum = maximum, Step = step})
...
local function numberBox(
    ...
    registerControl(name, "NumberBox", function()
        return tonumber(box.Text) or 0
    end, function(value, fire)
        local number = tonumber(value) or 0
        box.Text = tostring(number)
        if fire and callback then task.spawn(callback, number) end
    end)
...
local function textBox(
    ...
    registerControl(name, "Input", function()
        return box.Text
    end, function(value, fire)
        box.Text = tostring(value or "")
        if fire and callback then task.spawn(callback, box.Text) end
    end)
...
local function keybind(parent, name, defaultKey, callback)
    local current = defaultKey or "None"
    local listening = false
    local holder = create("Frame", {
        Name = "Keybind_" .. tostring(name),
    ...
    local label = create("TextLabel", {
        ...
        Size = UDim2.new(1,-105,1,0), Font = Enum.Font.GothamMedium,
        Text = tostring(name), TextColor3 = Theme.Text, TextSize = 11,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = holder
    })
    local button = create("TextButton", {
        AnchorPoint = Vector2.new(1,0.5), Position = UDim2.new(1,-8,0.5,0),
        Size = UDim2.fromOffset(82,22), BackgroundColor3 = Theme.Input,
        BorderSizePixel = 0, AutoButtonColor = false, Text = tostring(current),
        TextColor3 = Theme.Muted, TextSize = 10, Font = Enum.Font.GothamSemibold, Parent = holder
    })
    corner(button,3); local buttonStroke=stroke(button,Theme.Border,0.28)
    local function setKey(value, fire)
        current = tostring(value or "None"); button.Text = current
    ...
    local connection = UserInputService.InputBegan:Connect(function(input, processed)
        ...
        if input.UserInputType == Enum.UserInputType.Keyboard then
            listening = false; setKey(input.KeyCode.Name, true)
        elseif input.UserInputType == Enum.UserInputType.MouseButton1 then
            listening = false; setKey("Mouse1", true)
        elseif input.UserInputType == Enum.UserInputType.MouseButton2 then
            listening = false; setKey("Mouse2", true)
        end
    end)
    registerThemeRefresh(function()
        if holder.Parent then holder.BackgroundColor3=Theme.Row; holderStroke.Color=Theme.Border; label.TextColor3=Theme.Text; button.BackgroundColor3=Theme.Input; buttonStroke.Color=Theme.Border; button.TextColor3=Theme.Muted end
    end)
    bindRowHover(button, holder, holderStroke)
    registerControl(name, "Keybind", function() return current end, setKey)
    table.insert(keybindRegistry, {Name=name, Get=function() return current end, Set=setKey, Connection=connection})
    return holder
end
...
local function updateSidebarGeometry(
    ...
    sidebarProfile.Size = UDim2.new(1, -16, 0, 46)
    sidebarProfile.Position = UDim2.new(0, 8, 1, -8)
    sidebarAvatar.Visible = sidebarExpanded and not profileSettings.HideAvatar
    sidebarProfileName.Visible = sidebarExpanded
    sidebarProfileSub.Visible = sidebarExpanded
...
function Library:CreateWindow(config)
    ...
    -- Built-in utility pages. Existing pages and styling remain untouched.
    if not self._enhancementsInstalled then
        self._enhancementsInstalled = true
        local settingsTab = window:CreateTab({Name = "Settings", Icon = "Settings"})
        local profileSection = settingsTab:CreateSection({Name = "Profile"})
        local profileCard = infoCard(profileSection.Root, "", 58)
        local avatar = create("ImageLabel", {
            Position = UDim2.fromOffset(9,8), Size = UDim2.fromOffset(42,42),
            BackgroundColor3 = Theme.Input, BorderSizePixel = 0,
            Image = "rbxthumb://type=AvatarHeadShot&id=" .. tostring(player.UserId) .. "&w=150&h=150", Parent = profileCard
        })
        corner(avatar, 21)
        local profileName = create("TextLabel", {
            Position = UDim2.fromOffset(60,8), Size = UDim2.new(1,-70,0,20), BackgroundTransparency=1,
            TextColor3=Theme.Text, TextSize=11, Font=Enum.Font.GothamBold, TextXAlignment=Enum.TextXAlignment.Left, Parent=profileCard
        })
        local profileSub = create("TextLabel", {
            Position = UDim2.fromOffset(60,28), Size = UDim2.new(1,-70,0,18), BackgroundTransparency=1,
            TextColor3=Theme.Muted, TextSize=9, Font=Enum.Font.GothamSemibold, TextXAlignment=Enum.TextXAlignment.Left, Parent=profileCard
        })
        local function refreshProfile()
            local hidden=profileSettings.HideUsername; profileName.Text=hidden and "ZeHub Player" or player.DisplayName; profileSub.Text=hidden and "Username hidden" or ("@"..player.Name); avatar.Visible=not profileSettings.HideAvatar
            refreshSidebarProfile()
        end
        refreshProfile()
        toggle(profileSection.Root, "Hide Username", function(v) profileSettings.HideUsername=v; refreshProfile() end, profileSettings.HideUsername)
        toggle(profileSection.Root, "Hide Avatar", function(v) profileSettings.HideAvatar=v; refreshProfile() end, profileSettings.HideAvatar)
        local configSection = settingsTab:CreateSection({Name = "Configs"})
        local configName = textBox(configSection.Root, "Config Name", "Enter a config name", nil)
        local configInput = configName:FindFirstChildWhichIsA("TextBox", true)
        actionButton(configSection.Root, "Save Config", function() if configInput then saveConfig(configInput.Text) end end)
        actionButton(configSection.Root, "Load Config", function() if configInput then loadConfig(configInput.Text, true) end end)
        actionButton(configSection.Root, "Delete Config", function() if configInput then deleteConfig(configInput.Text) end end)
        local favoritesSection = settingsTab:CreateSection({Name = "Favorites"})
        infoCard(favoritesSection.Root, "Use Window:SetFavorite(Feature, true) to mark features. Favorites persist in saved configs.", 46)
        local utilitySection = settingsTab:CreateSection({Name = "UI"})
        toggle(utilitySection.Root, "Dark / Light Theme", function(v) applyTheme(v and "Light" or "Dark") end, currentThemeName == "Light")
        activePageName = nil
        for _, page in pairs(pages) do page.Root.Visible = false end
    end
...
-- ============================================================================
-- CONFIG / PROFILE / SEARCH / FAVORITES
-- ============================================================================
...
function WindowMethods:LoadConfig(name, fireCallbacks)
    return loadConfig(name, fireCallbacks)
end
...
function WindowMethods:SetProfilePrivacy(hideUsername, hideAvatar)
    profileSettings.HideUsername = hideUsername == true
    profileSettings.HideAvatar = hideAvatar == true
    return self
end

function WindowMethods:GetProfile()
    return {
        Username = profileSettings.HideUsername and "ZeHub Player" or player.Name,
        DisplayName = profileSettings.HideUsername and "ZeHub Player" or player.DisplayName,
...
function WindowMethods:SetFavorite(featureName, enabled)
    favoriteRegistry[tostring(featureName)] = enabled == true
    return self
end

function WindowMethods:IsFavorite(featureName)
    return favoriteRegistry[tostring(featureName)] == true
end

function WindowMethods:GetFavorites()
    local result = {}
    for name, enabled in pairs(favoriteRegistry) do if enabled then table.insert(result, name) end end
    table.sort(result); return result
end

function WindowMethods:Search(term)
    term = string.lower(tostring(term or ""))
    local results = {}
    if term == "" then return results end
    for name, control in pairs(controlRegistry) do
        if string.find(string.lower(name), term, 1, true) then table.insert(results, {Name=name, Kind=control.Kind}) end
    end
...
function SectionMethods:CreateKeybind(config)
    config = normalizeConfig(config, "Keybind")
    return keybind(self.Root, config.Name or "Keybind", config.Default or config.Key or "None", config.Callback)
end
...
SectionMethods.AddKeybind =
    SectionMethods.CreateKeybind
